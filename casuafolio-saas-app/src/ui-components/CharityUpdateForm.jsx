...
import { getCharity } from "../graphql/queries";
import { updateCharity } from "../graphql/mutations";
...
export default function CharityUpdateForm(props) {
  const {
    id: idProp,
    charity: charityModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = charityRecord
      ? { ...initialValues, ...charityRecord }
      : initialValues;
  ...
  const [charityRecord, setCharityRecord] = React.useState(charityModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getCharity.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getCharity
        : charityModelProp;
      setCharityRecord(record);
    };
    queryData();
  }, [idProp, charityModelProp]);
  React.useEffect(resetStateValues, [charityRecord]);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateCharity.replaceAll("__typename", ""),
            variables: {
              input: {
                id: charityRecord.id,
                ...modelFields,
      ...
      {...getOverrideProps(overrides, "CharityUpdateForm")}
      {...rest}
    >
      <SwitchField
        label="Is published"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || charityModelProp)}
