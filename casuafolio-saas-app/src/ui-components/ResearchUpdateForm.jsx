...
import { getResearch } from "../graphql/queries";
import { updateResearch } from "../graphql/mutations";
...
export default function ResearchUpdateForm(props) {
  const {
    id: idProp,
    research: researchModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = researchRecord
      ? { ...initialValues, ...researchRecord }
      : initialValues;
  ...
  const [researchRecord, setResearchRecord] = React.useState(researchModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getResearch.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getResearch
        : researchModelProp;
      setResearchRecord(record);
    };
    queryData();
  }, [idProp, researchModelProp]);
  React.useEffect(resetStateValues, [researchRecord]);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateResearch.replaceAll("__typename", ""),
            variables: {
              input: {
      ...
      {...getOverrideProps(overrides, "ResearchUpdateForm")}
      {...rest}
    >
      <SwitchField
        label="Is published"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || researchModelProp)}
