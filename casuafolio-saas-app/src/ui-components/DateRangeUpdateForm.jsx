...
import { getDateRange } from "../graphql/queries";
import { updateDateRange } from "../graphql/mutations";
const client = generateClient();
export default function DateRangeUpdateForm(props) {
  const {
    id: idProp,
    dateRange: dateRangeModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = dateRangeRecord
      ? { ...initialValues, ...dateRangeRecord }
      : initialValues;
    setStart(cleanValues.start);
    setEnd(cleanValues.end);
    setErrors({});
  };
  const [dateRangeRecord, setDateRangeRecord] =
    React.useState(dateRangeModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getDateRange.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getDateRange
        : dateRangeModelProp;
    ...
    queryData();
  }, [idProp, dateRangeModelProp]);
  React.useEffect(resetStateValues, [dateRangeRecord]);
  const validations = {
    start: [{ type: "Required" }],
    end: [],
  };
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateDateRange.replaceAll("__typename", ""),
            variables: {
              input: {
                id: dateRangeRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "DateRangeUpdateForm")}
      {...rest}
    >
      <TextField
        label="Start"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || dateRangeModelProp)}
